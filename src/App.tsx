...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (token && deviceId && !isActive && !hasPlaybackStarted) {
  ...
  }, [token, deviceId, isActive, hasPlaybackStarted]);
  ...
  if (!token) {
    ...
    return <LoginScreen onLogin={handleLogin} error={loginError} isConfigured={isConfigured} />;
  }
